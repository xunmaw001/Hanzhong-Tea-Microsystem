	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.chayeName" type="text" placeholder="茶叶名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('chaye','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('chaye','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='chayeTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in chayeTypesList" :key="index" :class="chayeTypes===item.codeIndex?'tabActive':''"
                                  @tap="chayeTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list-box6"
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
              :style='{"padding":"0","boxShadow":"0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"0 2% 20rpx 0","borderColor":"red","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"48%","borderStyle":"solid","height":"auto"}'>
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"200rpx"}'
                    mode="aspectFill" :src="product.chayePhoto?baseUrl+product.chayePhoto.split(',')[0]:''">
            </image>
            <view
                    ...
                    class="title">{{product.chayeName}}
            </view>
            <view v-if="product.chayeNewMoney"
                  :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                <text style="font-size: 35rpx;">￥</text>{{product.chayeNewMoney}}
            </view>
            <view v-else-if="product.chayeValue"
                  :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                {{product.chayeValue}}
            </view>
            <view style="display: flex;justify-content: space-between;margin: 3%;">
                <text v-if="isAuth('chaye','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if=" isAuth('chaye','删除')" class="cuIcon-delete"
                      @click.stop="onDeleteTap(product.id)">删除
                </text>
            </view>
        </view>
        <view @tap="onDetailTap(product)" v-if="index%6==1" class="list-item"
              :style='{"padding":"0","boxShadow":"0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"0 0 20rpx 2%","borderColor":"red","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"48%","borderStyle":"solid","height":"auto"}'>
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"200rpx"}'
                    mode="aspectFill" :src="product.chayePhoto?baseUrl+product.chayePhoto.split(',')[0]:''">
            </image>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==4" class="list-item"
              :style='{"padding":"0","boxShadow":"0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"0 0 20rpx","borderColor":"red","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'
              style="display: flex;">
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"200rpx","borderStyle":"solid","height":"200rpx"}'
                    mode="aspectFill" :src="product.chayePhoto?baseUrl+product.chayePhoto.split(',')[0]:''">
            </image>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==5" class="list-item"
            ...
            <view class="list-item-body" style="flex: 1;">
                ...
                <view v-if="product.chayeNewMoney"
                      :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                    <text style="font-size: 35rpx;">￥</text>{{product.chayeNewMoney}}
                </view>
                <view v-else-if="product.chayeValue"
                      :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 茶叶类型 字段 字典表查询方法
            async chayeTypesSelect(){
                let chayeTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'chaye_types',
                });
                chayeTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.chayeTypesList = chayeTypes.data.list
            },
            //当前表的 茶叶类型字段类别搜索
			chayeTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.chayeTypes != 0) {
                    params.chayeTypes = this.chayeTypes
                }
                let _this = this
                let res = await this.$api.list(`chaye`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chaye', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.chayeShenheTime=shijian;
                            await _this.$api.update(`chaye`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.chayeName) {
                    searchForm['chayeName'] = this.searchForm.chayeName
                }
				if (this.searchForm.chayeCangku) {
                    searchForm['chayeCangku'] = this.searchForm.chayeCangku
                }

                let res = await this.$api.list(`chaye`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.chayeContent!=null){
                        item.chayeContent = item.chayeContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
