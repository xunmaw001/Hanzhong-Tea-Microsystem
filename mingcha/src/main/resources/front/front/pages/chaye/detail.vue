		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.chayeNewMoney>1000? 'width: 450rpx;':detail.chayeNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.chayeNewMoney}}</text>
						<text style="font-size: 20rpx;color: #fff;margin-left: 10rpx;">
							<text v-if="detail.chayeOldMoney!=null">原价￥
								<text style="text-decoration:line-through">{{detail.chayeOldMoney}}</text>
							</text>
						</text>
				</view>
			</view>
			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.chayeName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.chayeContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 评价 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chayeId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getchayeCollection()
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chaye', this.chayeId);
                    this.detail = res.data
					this.detail.chayeContent = this.detail.chayeContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.chayePhoto ? this.detail.chayePhoto.split(",") : [];
				},
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`chayeCollection`, params);
					if (res.data.list.length == 1) {
						let chayeCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('chayeCollection', JSON.stringify([chayeCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getchayeCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chayeCollection', {
									yonghuId: _this.user.id,
									chayeId: _this.detail.id,
									chayeCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('chayeCommentback', {
						page: mescroll.num,
						limit: mescroll.size,
						chayeId: this.chayeId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('chaye', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chayeYuyue', {
									yonghuId: _this.user.id,
									chayeId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/chayeYuyue/list');
							}
						}
					});
				},
