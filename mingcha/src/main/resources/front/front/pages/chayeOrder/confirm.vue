    ...
    <view class="content">
        <form>
            ...
            <view v-for="(item,index) in orderGoods " v-bind:key="index" class="cu-form-group">
                <image class="avator" :src="baseUrl+item.chayePhoto" mode=""></image>
                <view class="title" style="width: 75%;">
                    <view style="margin-top: -50rpx;">{{item.chayeName}}</view>
                    <view >
                        x{{item.buyNumber}}
                        <text style="margin-left: 30upx;color: red;">￥{{item.chayeNewMoney}}</text>
                    </view>
                </view>
            </view>
            <view @tap="onAddressTap" class="cu-form-group">
                ...
                <view>
                    <text v-if="chayeOrderPaymentTypes == 1">原价：￥{{(maxNewMouey).toFixed(2)}}</text>
                    <view v-if="chayeOrderPaymentTypes == 1"></view>
                    <text v-if="chayeOrderPaymentTypes == 1">折扣价：￥{{(maxNewMouey * zhekou).toFixed(2)}}</text>
                    <text v-if="chayeOrderPaymentTypes == 2">{{(maxNewMouey).toFixed(2)}}积分</text>
                </view>
            </view>
        </form>
        ...
        async onLoad(options) {
            ...
            if(this.orderGoods.length>0){
                for (let i = 0; i < this.orderGoods.length; i++) {
                    this.maxNewMouey = this.maxNewMouey + parseFloat(this.orderGoods[i].chayeNewMoney) * this.orderGoods[i].buyNumber
                }
            }
                        ...
                        if (res.confirm) {
                            if(_this.addresszhi == null){
							 ...
							 let data = {
								 ...
								 yonghuId: _this.user.id,
								 chayeOrderPaymentTypes:  _this.chayeOrderPaymentTypes,
							 }
							await _this.$api.requestConditionDataGet('chayeOrder','order',null,data);
                            _this.$utils.jump('/pages/chayeOrder/list');
                        }
                    }
                });
            },
