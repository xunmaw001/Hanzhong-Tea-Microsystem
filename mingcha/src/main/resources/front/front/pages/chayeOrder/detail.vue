	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chayeOrder', this.chayeOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.chayeOrderPhoto ? this.detail.chayeOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chayeOrderYuyue', {
									yonghuId: _this.user.id,
									chayeOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/chayeOrderYuyue/list');
							}
						}
					});
				},
