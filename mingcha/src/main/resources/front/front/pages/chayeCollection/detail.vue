	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chayeCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chayeCollection', this.chayeCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.chayeCollectionPhoto ? this.detail.chayeCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chayeCollectionYuyue', {
									yonghuId: _this.user.id,
									chayeCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/chayeCollectionYuyue/list');
							}
						}
					});
				},
