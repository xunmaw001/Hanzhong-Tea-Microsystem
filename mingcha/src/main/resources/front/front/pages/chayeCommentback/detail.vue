	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chayeCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chayeCommentback', this.chayeCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.chayeCommentbackPhoto ? this.detail.chayeCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chayeCommentbackYuyue', {
									yonghuId: _this.user.id,
									chayeCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/chayeCommentbackYuyue/list');
							}
						}
					});
				},
