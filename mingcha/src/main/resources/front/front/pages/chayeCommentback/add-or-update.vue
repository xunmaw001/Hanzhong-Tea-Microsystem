        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.chayeCommentbackText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`chayeCommentback`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.chayeId){
                this.ruleForm.chayeId = options.chayeId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.chayeCommentbackText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`chayeCommentback`, this.ruleForm);
                } else {
                    await this.$api.save(`chayeCommentback`, this.ruleForm);
                }
