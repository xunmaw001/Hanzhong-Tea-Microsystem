	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.lishijieshaoName" type="text" placeholder="茗茶历史名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('lishijieshao','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('lishijieshao','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.lishijieshaoPhoto?baseUrl+product.lishijieshaoPhoto.split(',')[0]:''">
            </image>
            <view
                    ...
                    class="title">{{product.lishijieshaoName}}
            </view>
            <view v-if="product.lishijieshaoNewMoney"
                ...
                <text style="font-size: 35rpx;">￥</text>{{product.lishijieshaoNewMoney}}
            </view>
            <view v-else-if="product.lishijieshaoValue"
                  :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                {{product.lishijieshaoValue}}
            </view>
            <view style="display: flex;justify-content: space-between;margin: 3%;">
                <text v-if="isAuth('lishijieshao','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if=" isAuth('lishijieshao','删除')" class="cuIcon-delete"
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.list(`lishijieshao`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('lishijieshao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.lishijieshaoShenheTime=shijian;
                            await _this.$api.update(`lishijieshao`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.lishijieshaoName) {
                    searchForm['lishijieshaoName'] = this.searchForm.lishijieshaoName
                }

                let res = await this.$api.list(`lishijieshao`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.lishijieshaoContent!=null){
                        item.lishijieshaoContent = item.lishijieshaoContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
