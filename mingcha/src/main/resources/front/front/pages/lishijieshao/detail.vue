			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.lishijieshaoName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				<view class="xiangqing">
					<view class="shangpinxiangqing">
						...
						<view class="app">
							{{detail.lishijieshaoUuidNumber}}
						</view>
					</view>
				</view>
				<view class="xiangqing" @tap="download(baseUrl + detail.lishijieshaoFile)">
					<view class="shangpinxiangqing">
						<view class="box">
							资料：
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.lishijieshaoContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.lishijieshaoId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('lishijieshao', this.lishijieshaoId);
                    this.detail = res.data
					this.detail.lishijieshaoContent = this.detail.lishijieshaoContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.lishijieshaoPhoto ? this.detail.lishijieshaoPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('lishijieshaoYuyue', {
									yonghuId: _this.user.id,
									lishijieshaoId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/lishijieshaoYuyue/list');
							}
						}
					});
				},
